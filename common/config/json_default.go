...
func (s *JsonConfig) Check() {
	...
	if s.MQTT.Client.Port == "" {
		s.MQTT.Client.Port = "1883"
	}
	if s.MQTT.Client.Server == "" {
		s.MQTT.Client.Server = "47.99.33.224" //默认连接服务器
	}
	if s.MQTT.Client.UserName == "" {
		s.MQTT.Client.UserName = "soe"
	}
	if s.MQTT.Client.Password == "" {
		s.MQTT.Client.Password = "88951566"
	}
